...
def main():
    ...
    # Read from a file
    file_name = "/home/pi/Documents/picture.jpg"
    ...
    coins = euro_detect(rgb)

    # We extract the values of the coins in the coins_list to then calculate the sum of money

    coins_list = []

    if coins is not None:
        for c in coins:
            results = list(c.result['p'].items())
            results.sort(key=lambda data: data[1], reverse=True)
            valeur, proba = results[0]
            # If the probability of correspondence is lower than 20%, we don't take the coin into account
            if proba >= 0.20:
                coins_list.append(valeur)
